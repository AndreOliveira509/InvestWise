...
export default function Dashboard() {
  ...
  const [form, setForm] = useState({ description: '', amount: '', category: 'alimentacao', date: new Date().toISOString().split('T')[0] });
  ...
  const weekSpending = useMemo(() => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const daySpent = expenses.filter(e => e.date === dayStr).reduce((s, e) => s + parseFloat(e.amount || 0), 0);
  ...
  const pieData = useMemo(() => {
    ...
    return categories.map(c => ({
      name: c.name,
      value: expenses.filter(e => e.category === c.id).reduce((s, e) => s + parseFloat(e.amount || 0), 0),
      color: c.color
    })).filter(d => d.value > 0);
  }, [expenses]);
  ...
  const barData = useMemo(() => (
    categories.map(c => ({
      name: c.name,
      lucro: Math.max(0, budget / 7 - expenses.filter(e => e.category === c.id).reduce((s, e) => s + parseFloat(e.amount || 0), 0)),
      prejuizo: expenses.filter(e => e.category === c.id).reduce((s, e) => s + parseFloat(e.amount || 0), 0)
    }))
  ), [expenses, budget]);
  ...
  const handleAddInvestment = e => {
    ...
    setInvestments([{ id: Date.now(), ...investmentForm, value: parseFloat(investmentForm.value), change: 0 }, ...investments]);
            ...
            <section className={styles.transactionsArea}>
              <div className={styles.transactionsCard}>
                ...
                <div className={styles.transactionList}>
                  {filteredExpenses.length > 0 ? (filteredExpenses.map(exp => { const category = categories.find(c => c.id === exp.category) || {}; return (
                    <div key={exp.id} className={styles.transactionItem}>
                      ...
                      <div className={styles.transactionDetails}><span className={styles.transactionDesc}>{exp.description}</span><span className={styles.transactionDate}>{new Date(exp.date).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' })}</span></div>
                      <span className={styles.transactionAmount}>- R$ {parseFloat(exp.amount || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
